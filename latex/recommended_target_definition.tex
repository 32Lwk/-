...
\noindent\textbf{接触・配信の推奨区分.}\par\smallskip
\begingroup
\setlength{\leftmargini}{1.5em}%
\setlength{\labelsep}{0.6em}%
\begin{description}\setlength{\itemsep}{0.35em}\setlength{\parsep}{0pt}
  \item[\textbf{優先的接触（推奨）}] 購入スコアが\textbf{上位5--10\,\%}かつ、オフライン期待利益代理（式\eqref{eq:profit}）で\textbf{当該顧客にとって最良の処置}がインセンティブ付きでない、または予算・接触上限内で割り当て可能な場合。制約付き配信では貪欲割当の結果に従う（第\ref{sec:opeval}節）。
  \item[\textbf{条件付き接触}] スコアは中位だが在庫処理・季節キャンペーン等の\textbf{ビジネス上の理由}があり、RCTまたはホールドアウトで\textbf{害がない}ことが確認されているセル。
  \item[\textbf{接触回避・ホールドアウト}] スコア下位で期待利益代理が負、または同一顧客への\textbf{過剰接触}が予想される場合。RCTのコントロール臂として無処置を必ず確保する。
  \item[\textbf{禁止（ガバナンス）}] ガバナンス上レビュー前に、地域カテゴリ等の\textbf{禁止・要レビュー変数のみ}を理由に不利益な差別的配信を行わない（第\ref{sec:governance}節）。
\end{description}
\endgroup
